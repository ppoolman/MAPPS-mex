...
#include "_SetUpConnection_client_code.h"
	...
	if(Buffer == NULL)
	{
		...
		mexErrMsgIdAndTxt("MEX:ReloadOptoTransform:UnableToConnectClient", "Unable to connect to shared memory buffer (client state)! \n");
	}
	...
	if(Prepare(global, client, EDS_OptoSupMessage) == FALSE)
	{
        ...
        DisconnectFromGlobal(h1, global);
		mexErrMsgIdAndTxt("MEX:ReloadOptoTransform:ClientBlocked", "Server is still blocking the client channel! \n");
	}
    ...
    sprintf(Command_string, "sup_reload_transform: ");
    ...
    if(Execute(global, client, (int)bufferSize) == FALSE)
    {
        DisconnectFromClient(h2, client);
        DisconnectFromGlobal(h1, global);
        mexErrMsgIdAndTxt("MEX:ReloadOptoTransform:ServerTimedOut", "SERVER TIMED OUT! \n");
    }
    ...
    if(strncmp(client->Result, "success", 7) != 0)
	    mexPrintf("Details (mxReloadOptoTransform): %s\n", client->Details); 
