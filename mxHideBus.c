...
#include "_SetUpConnection_client_code.h"
	...
	if(Buffer == NULL)
	{
		...
		mexErrMsgIdAndTxt("MEX:HideBus:UnableToConnectClient", "Unable to connect to shared memory buffer (client state)! \n");
	}
	...
	if(Prepare(global, client, EDS_SetVisibility) == FALSE)
	{
        ...
        DisconnectFromGlobal(h1, global);
		mexErrMsgIdAndTxt("MEX:HideBus:ClientBlocked", "Server is still blocking the client channel! \n");
	}
    ...
    if(Execute(global, client, (int)bufferSize) == FALSE)
    {
        DisconnectFromClient(h2, client);
        DisconnectFromGlobal(h1, global);
        mexErrMsgIdAndTxt("MEX:HideBus:ServerTimedOut", "SERVER TIMED OUT! \n");
    }
