...
#include "_SetUpConnection_client_code.h"
	...
	if(Buffer == NULL)
	{
		...
		mexErrMsgIdAndTxt("MEX:CancelOptoTrigger:UnableToConnectClient", "Unable to connect to shared memory buffer (client state)! \n");
	}
	...
	if(Prepare(global, client, EDS_OptoClientMessage) == FALSE)
	{
        ...
        DisconnectFromGlobal(h1, global);
		mexErrMsgIdAndTxt("MEX:CancelOptoTrigger:ClientBlocked", "Server is still blocking the client channel! \n");
	}
    ...
    sprintf(Command_string, "cancel_trigger: Trigger%d", trigger);
    ...
    if(Execute(global, client, (int)bufferSize) == FALSE)
    {
        DisconnectFromClient(h2, client);
        DisconnectFromGlobal(h1, global);
        mexErrMsgIdAndTxt("MEX:CancelOptoTrigger:ServerTimedOut", "SERVER TIMED OUT! \n");
    }
    ...
    if(strncmp(client->Result, "success", 7) != 0)
	    mexPrintf("Details (mxCancelOptoTrigger): %s\n", client->Details); 
